...
public class MessageController {
    ...
    public ApiResponse<List<Message>> getPersistedOfflineMessage(@RequestAttribute(value = "UserId", required = false) String userId) {
        ...
        logger.debug("persisted offline messages: {}", offlineMessages);
    ...
    public ApiResponse<List<Message>> getOfflineMessage(
        ...
        // 批量入库：将 messages 序列化为 JSON 数组并发送到 Kafka，消费端负责批量入库
        if (messages != null && !messages.isEmpty()) {
            try {
                String jsonArray = MAPPER.writeValueAsString(messages);
                kafkaProducer.sendMessage(jsonArray);
            } catch (Exception e) {
                logger.error("failed to send offline messages to kafka for user {}", userId, e);
            }
        }
